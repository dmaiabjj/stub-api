...
describe('[DATA/SOURCE/LOCAL]]', function () {
  it('should return the user authenticated', async () => {
    ...
    const token = await local.authenticate(new Auth(config.credentials.diverso.USER,
      config.credentials
  ...
  it('should return null instead of a user', async () => {
    const expected = null;
    const token = await local.authenticate(new Auth("", ""));
    expect(token).to.deep.equal(expected);
  });
